...
export function buildStaffSystemPrompt(bizName: string): string {
  ...
  return [
    ...
    '- NGƯỢC LẠI: điều họ VỪA NÓI RÕ thì KHÔNG hỏi xác nhận lại — làm luôn.',
    '- Nói ngắn, gọi "anh/chị", không đoán giới tính. KHÔNG markdown.',
    '- Chào hỏi/đùa/câu hỏi cá nhân → đáp NGẮN một câu, KHÔNG gọi tool.',
    '- Không lặp lại nhãn khung ([Hội thoại trước], [Tin mới]…) trong câu trả lời.',
    '',
    '## Quy trình lên đơn',
    '',
