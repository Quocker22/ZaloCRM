    ...
    <template v-else>
      ...
      <div class="nka-ds" :class="{ 'nka-mo': dangTai && items.length > 0 }" role="log" aria-label="Nhật ký app máy in">
        <div
          ...
          :key="k.id"
          class="nka-dong"
          :class="`nka-dong--${kieuSuKienApp(k.suKien).tong}`"
        >
          ...
          <span class="nka-noi-dung">{{ k.noiDung }}</span>
        </div>
...
import {
  ...
  type MayIn, type NhatKyApp, type ThamSoNhatKyApp,
...
watch([tuKhoa, mayInId, khoang, nhom], () => {
  loiTaiXuong.value = '';
  void taiLai();
});
...
.nka-dong--do { border-left-color: var(--at-atlas-danger, #f04438); background: #fff8f7; }
.nka-dong--vang { border-left-color: var(--at-atlas-warning, #f5a524); }
